...
function dl_lastest_release() {
    VERSION=$(curl -fsSL $proxy https://api.github.com/repos/$1/$2/releases/latest | jq .tag_name |  sed 's/\"//g')
    DOWNLOAD_URL=$(curl -fsSL $proxy https://api.github.com/repos/$1/$2/releases/latest | jq .assets | grep browser_download_url | sed -E 's/.*"(.*)".*/\1/')
...
function dl_custom_release() {
    DOWNLOAD_URL=$(curl -fsSL $proxy https://api.github.com/repos/$1/$2/releases/tags/$3 | jq .assets | grep browser_download_url | sed -E 's/.*"(.*)".*/\1/')
...
function dl_all_release() {
    VERSIONS=$(curl -fsSL $proxy https://api.github.com/repos/$1/$2/releases | jq .tag_name |  sed 's/\"//g')
    ...
    do
        if [ -d "$VERSION" ];then
            ...
            DOWNLOAD_URL=$(curl -fsSL $proxy https://api.github.com/repos/$1/$2/releases/tags/$VERSION | jq .assets | grep browser_download_url | sed -E 's/.*"(.*)".*/\1/')
            for DOWNLOAD_FILE in $DOWNLOAD_URL
            do
                echo -e "\033[33m[Notice]\033[0mDownloading \033[32m$DOWNLOAD_FILE\033[0m"
                curl -O -fSL $proxy -# $DOWNLOAD_FILE
